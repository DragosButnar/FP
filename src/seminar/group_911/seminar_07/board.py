...
def get_position_on_board(board, row, col: int):
    ...
    :param col:
    :return: X, 0, None
    Raise ValueError if position outside the board
    """
    if row not in [0,1,2] or col not in [0,1,2]:
        raise ValueError(f"Position not on board - ({str(row)}, {str(col)})")
...
def str_board(board):
    """
    Return the board str representation
...
def make_move_on_board(board, symbol, row, column):
    ...
    :param symbol: One of ['x', '0']
    :param row column: Position to play
    :return: None
    Raise ValueError if invalid symbol, already occupied square, play outside of board
    """
    ...
    if get_position_on_board(board, row, column) is not None:
        ...
        # same as raise ValueError(f"Cannot overwrite board at ({str(row)}, {str(column)})")
...
def test_board():
    ...
    # Check if moving outside the board causes ValueError
    try:
    ...
    try:
        make_move_on_board(board, '0', 0, 0)
        assert False
    except ValueError:
        assert True
def create_board():
