...
def human_move(game_board, row, col):
    """
    Record the human move on board
    :param game_board:
    :param row:
    :param col:
    :return:
...
def computer_move(game_board):
    """
    Determine where computer should play
    :param board:
    :return:
    """
    for row in [0,1,2]:
        for col in [0,1,2]:
            if board.get_position_on_board(game_board, row, col):
                row, col = board.make_move_on_board(game_board, '0', row, col)
                print(f"Computer move at ({str(row)}, {str(col)})")
                return row, col
import board
